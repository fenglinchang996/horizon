...
| TZ        | Specifies your time zone. Most Unix-like systems maintain the computer’s internal clock in _Coordinated Universal Time_ (_UTC_) and then display the local time by applying an offset specified by this variable. |
| USER      | Your username.                                                                                                                                                                                                    |
...
When we log on to the system, the shell program (bash, zsh, fish etc.) starts and reads a series of configuration scripts called _startup files_, which define the default environment shared by all users. This is followed by more startup files in our home directory that define our personal environment. The exact sequence depends on the type of shell session being started. There are 2 kinds:
...
- The `PATH` variable is often set by the shell's startup file. The specific file and method for setting the PATH variable depend on the shell type and the operating system distribution.

## Modifying the Environment

### Which files should we modify?

- (In general) To add directories to your `PATH` or define additional environment variables, place those changes in `.bash_profile` (`.zprofile`).
- For everything else, place the change in `.bashrc` (`.zshrc`).

### Activating our changes

The changes we have made to our shell's config file will not take effect until we close our terminal session and start a new one because the config file is only read at the beginning of a session. However, we can force the shell to reread the modified config file with the following command:
