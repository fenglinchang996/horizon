...
- `cp file1 file2` —> If `file2` exists, it is overwritten with the contents of `file1`. If `file2` does not exist, it created.
...
Before overwriting an existing file, prompt the user for confirmation. If this option is not specified, `mv` will silently (meaning there will be no warning) overwrite files.
