...
### `ps`: Report a sanpshot of current processes (process status)

```bash
$ ps
```

By default, `ps` just show the processes associated with the current terminal session.
...
- TTY: Short for *teletype*, refering to the *controlling terminal* for the process

- TIME: The amount of CPU time consumed by the process

- STAT: Short for *state*, revealing the current status of the process
...
The `fg` command followed by a `$` and the job number (called a *jobspec*) does the trick.
...
**Interrupt**. This performs the same function as a `CTRL-C` sent from the terminal. It will usually terminate a program.
...
- `shutdown`:
   ...
   `time` can be word `now` (inidicating an immediate shutdown) or a future time in one of 2 formats: `number` or `yymmddhhmm`.